...
# Универсальная функция парсинга даты (работает на macOS и Linux)
parse_doc_date() {
  local date_str="$1"
  # Извлекаем только дату (YYYY-MM-DD), отбрасываем время если есть
  ...
  if date --version >/dev/null 2>&1; then
    date -d "$date_only" +%s 2>/dev/null
  # BSD date (macOS)
  else
    date -j -f "%Y-%m-%d" "$date_only" "+%s" 2>/dev/null
  fi
}
...
audit_project() {
  ...
  # Убираем последний "-o" если массив не пуст
  ...
  total_issues=$((total_issues + spec_issues))
  
  # ─── 4. Устаревшие документы ────────────────────────────────────────────────────
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "📅 Устаревшие документы (старше $doc_stale_days дней)"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  
  local stale_count=0
  local docs_dir="$target/docs"
  
  if [[ -d "$docs_dir" ]]; then
    # Собираем устаревшие документы
    local -a stale_docs=()
    
    while IFS= read -r -d '' doc_file; do
      # Ищем строку с Last verified
      local date_line
      date_line=$(grep -E '^\> \*\*Last verified:\*\*' "$doc_file" 2>/dev/null | head -1)
      
      [[ -z "$date_line" ]] && continue
      
      # Извлекаем дату
      local date_str
      date_str=$(echo "$date_line" | sed 's/.*Last verified:\*\*[[:space:]]*//')
      
      [[ -z "$date_str" ]] && continue
      
      # Парсим дату в epoch
      local epoch
      epoch=$(parse_doc_date "$date_str")
      ...
      if [[ $days -gt $doc_stale_days ]]; then
        local rel_path="${doc_file#$target/}"
        stale_docs+=("$days|$rel_path")
      fi
    done < <(find "$docs_dir" -name "*.md" -type f -print0 2>/dev/null)
    
    # Сортируем по возрасту (старшие первые) и берём топ-N
    if [[ ${#stale_docs[@]} -gt 0 ]]; then
      local sorted_stale
      sorted_stale=$(printf '%s\n' "${stale_docs[@]}" | sort -t'|' -k1 -rn | head -n "$doc_stale_max")
      
      while IFS='|' read -r days path; do
        echo "  ⚠ $path ($days дней)"
        stale_count=$((stale_count + 1))
      done <<< "$sorted_stale"
      
      local total_stale=${#stale_docs[@]}
      if [[ $total_stale -gt $doc_stale_max ]]; then
        local hidden=$((total_stale - doc_stale_max))
        echo ""
        echo "  ... и ещё $hidden документов"
      fi
      echo ""
    else
    ...
    echo "  ℹ docs/ не найден — проверка пропущена"
    echo ""
  fi
